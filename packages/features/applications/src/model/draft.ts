...
export function sanitizeDraft(
  ...
  return Object.fromEntries(
    Object.entries(fields).filter(
      ([key]) => !(NEVER_PERSIST as readonly string[]).includes(key),
    ),
  );
}
export function makeDraft(
  ...
  return {
    ...
    fields: sanitizeDraft(fields),
  };
}
export function parseDraft(raw: string | null): ApplicationDraft | null {
  if (!raw) return null;
  try {
    const value = JSON.parse(raw) as Partial<ApplicationDraft>;
    if (
      value.version !== DRAFT_SCHEMA_VERSION ||
      typeof value.step !== "string" ||
      !(WIZARD_STEPS as readonly string[]).includes(value.step) ||
    ...
    return {
      version: DRAFT_SCHEMA_VERSION,
      step: value.step as WizardStep,
      updatedAt: typeof value.updatedAt === "string" ? value.updatedAt : "",
      fields: sanitizeDraft(value.fields as Record<string, unknown>),
    };
  } catch {
    return null;
  }
