...
export default function ApplicationDetailPage(): React.ReactElement {
  ...
  if (isError || data === undefined) return <Box xcss={pageStyles.base}><SectionMessage appearance="error" title={t("errors.not_found")} headingLevel="h3">{t("errors.not_found")}</SectionMessage></Box>;
  const application = data.application.application;
  const history = data.history?.history ?? [];
  return <Box xcss={pageStyles.base}><Stack space="space.500"><Inline spread="space-between" alignBlock="center"><Stack space="space.100"><Heading size="large" as="h1">{application.processingCode}</Heading><Text color="color.text.subtle" size="small">{t("application.id")}: {application.id}</Text></Stack><ApplicationStatusBadge status={application.status} /></Inline><SectionMessage title={t("application.history")} headingLevel="h3"><Stack space="space.100">{history.map((event, index) => <Inline key={`${event.at}-${index}`} space="space.150" alignBlock="start"><Text size="small" color="color.text.subtle">{new Date(event.at).toLocaleString()}</Text><Text size="small">{event.actorKind} → {event.toStatus}{event.reason !== null ? `: ${event.reason}` : ""}</Text></Inline>)}</Stack></SectionMessage>{data.partial.length > 0 && <SectionMessage appearance="warning" title={t("errors.generic")} headingLevel="h3">{t("errors.generic")}</SectionMessage>}<Button appearance="primary" href="/applications">{t("actions.back")}</Button></Stack></Box>;
}
