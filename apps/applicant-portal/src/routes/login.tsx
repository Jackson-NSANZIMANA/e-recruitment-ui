...
/**
 * Citizen sign-in: National ID, then a one-time code.
 *
 * There is no password. identity-service issues a 5-minute OTP challenge and
 * returns an opaque, revocable session token that dies at the edge (ADR-018).
 */
export default function LoginPage(): React.ReactElement {
  ...
  // The edge set an httpOnly cookie this code cannot read, so a full document
  // load is what makes AuthProvider re-probe GET /edge/v1/session and see it.
  useEffect(() => {
    if (otp.status === "verified") window.location.assign("/");
  }, [otp.status]);

  const awaitingCode =
    otp.status === "challenged" || otp.status === "rejected" || otp.status === "verifying";
      ...
      <Box backgroundColor="color.background.input" xcss={cardStyles.base}>
        <Stack space="space.400">
          ...
          {!awaitingCode && otp.status !== "verified" && (
            <Form<NationalIdFormValues>
              onSubmit={(values) => applicant.requestCode(values.nationalId.trim())}
            >
                  ...
                  <Stack space="space.300">
                    ...
                    <FormFooter>
                      <LoadingButton
                        type="submit"
                        appearance="primary"
                        isLoading={submitting || otp.status === "requesting"}
                        shouldFitContainer
                      >
                        {t("auth.sign_in")}
                      </LoadingButton>
          ...
          {awaitingCode && (
            <Form<OtpFormValues> onSubmit={(values) => applicant.submitCode(values.otp.trim())}>
              {({ formProps, submitting }) => (
                <form {...formProps}>
                  <Stack space="space.300">
                    <Field name="otp" label="One-time code" isRequired>
                      {({ fieldProps }) => (
                        <TextField
                          {...fieldProps}
                          type="text"
                    ...
                    {/*
                      Both figures are LOCAL approximations - the API exposes
                      neither the server's attempt count nor its TTL. Shown so a
                      citizen at a kiosk is not surprised, never as a verdict.
                    */}
                    <Text size="small" color="color.text.subtle">
                      {applicant.attemptsLeft} attempts remaining, {secondsLeft}s left
                    </Text>
                    <FormFooter>
                      <Inline space="space.200">
                        <LoadingButton
                          type="submit"
                          appearance="primary"
                          isLoading={submitting || otp.status === "verifying"}
                          isDisabled={!applicant.canSubmitCode}
                        >
                          {t("actions.confirm")}
                        </LoadingButton>
